<x-layouts.app title="Preview Laporan">
    ...
    <div class="mb-8 flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
            ...
            <h1 class="text-[32px] font-bold text-[#1C1C1E] tracking-tight">Preview Laporan Stok & Aset</h1>
            <p class="text-[15px] text-[#8E8E93] mt-1">Tinjau rekapitulasi nilai modal dan estimasi keuntungan · {{ $totalProducts }} produk</p>
        </div>

        {{-- Tombol Aksi Export --}}
        <div class="flex items-center gap-2 shrink-0">
            <a href="{{ route('reports.pdf') }}" class="inline-flex items-center gap-2 bg-[#FF3B30] text-white text-[14px] font-semibold px-4 py-2.5 rounded-full hover:bg-[#D32F2F] transition-colors shadow-sm">
            ...
            <a href="{{ route('reports.excel') }}" class="inline-flex items-center gap-2 bg-[#114F11] text-white text-[14px] font-semibold px-4 py-2.5 rounded-full hover:bg-[#0D3D0D] transition-colors shadow-sm">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3M4 4h16v16H4V4z" /></svg>
                Export Excel
            </a>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div class="ios-card p-5">
            ...
            <h3 class="text-[24px] font-bold text-[#1C1C1E] tabular-nums">
        ...
        <div class="ios-card p-5">
            ...
            <h3 class="text-[24px] font-bold text-[#114F11] tabular-nums">
    ...
    {{-- Tabel Laporan dengan Kolom Finansial --}}
    <div class="ios-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left">
                <thead>
                    <tr class="border-b border-black/5 bg-[#F2F2F7]/50">
                ...
                <tbody class="divide-y divide-black/5">
                    @foreach($products as $product)
                        ...
                        <tr class="hover:bg-black/[0.015]">
                            ...
                            <td class="px-5 py-3.5">
                                <span class="text-[12px] px-2.5 py-1 rounded-full bg-[#F2F2F7] text-[#3A3A3C] font-medium">{{ $product->category->name }}</span>
                            </td>
                ...
                <tfoot class="border-t-2 border-black/10 bg-[#F2F2F7]">
                    <tr>
